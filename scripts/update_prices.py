...
def scrape_pegasus_api(origin_code, destination_code, depart_date):
    ...
    try:
        ...
        log_debug(f"STATUS {response.status_code} for {origin_code}->{destination_code} {depart_date}")
        log_debug(f"CONTENT-TYPE {response.headers.get('content-type')}")

        if not response.text.strip():
            log_debug("Empty response body")
            return None
        ...
        if cheapest_fare and cheapest_fare.get("amount") is not None:
            amount = float(cheapest_fare["amount"])
            log_debug(f"cheapestFare.amount = {amount}")
        ...
        if prices:
            amount = min(prices)
            log_debug(f"fallback price = {amount}")
            return round(amount, 2)
